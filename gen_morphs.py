...
def make_mipgan2_morphs(PERMUTATIONS, DST_SUFFIX, SRC_DIR, LATENTS_DIR, arg_dst_path, generator, cropper, alpha):
    ...
    for f1, f2 in PERMUTATIONS:
        ...
        f1 = f1[:-4]
        f2 = f2[:-4]
        m_name = '_'.join((f1, f2)) + DST_SUFFIX
...
def main():
    ...
    if args.opencv:
        for alpha in ALPHA_LIST:
            ...
            make_opencv_morphs(PERMUTATIONS, SRC_DIR, dst_path, detector, predictor, fa, alpha)

    # FaceMorpher Morphs
    if args.facemorpher:
        for alpha in ALPHA_LIST:
            dst_path = make_dst_path(args.dst, 'facemorpher', alpha)
            make_facemorpher_morphs(PERMUTATIONS, SRC_DIR, WIDTH, HEIGHT, dst_path, alpha)
